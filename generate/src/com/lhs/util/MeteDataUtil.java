...
public class MeteDataUtil {
	...
	public static Map<String, String> queryTables(ReadProperties prop) throws SQLException, IOException {
		...
		return needGenerateTables(returnMap,prop);
	}

	public static Map<String, String> needGenerateTables(Map<String, String> dbMap, ReadProperties prop) throws IOException {
		String need_generate_tables = prop.getValue("need_generate_tables");
		if (need_generate_tables != null && !"".equals(need_generate_tables)) {
			String[] needs = need_generate_tables.split(",");
			Map<String, String> returnMap = new HashMap<String, String>();
			for (Map.Entry<String, String> map : dbMap.entrySet()) {
				for (String table : needs) {
					if (map.getKey().toLowerCase().equals(table)) {
